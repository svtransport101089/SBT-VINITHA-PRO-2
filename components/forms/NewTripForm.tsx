...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ invoiceIdToLoad, onSaveSuccess, onCancel }) => {
            ...
            try {
                ...
                if (invoiceIdToLoad) {
                    ...
                    if (loadedInvoice) {
                        ...
                        // Also load available memos for this customer
    ...
    const handleCustomerChange = async (customerName: string) => {
        if (invoice.id) return; // Don't allow customer change on existing invoices
    ...
    // Memos to display are available ones plus already selected ones for editing
